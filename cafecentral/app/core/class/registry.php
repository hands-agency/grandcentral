 ...
 */
	...
	//	pour les deux environnements
		...
		//	on cherche les structures disponibles et on les mets dans le registre
			$structures = new bunch(null, null, $env);
			$datas = $db->querydata('structure');
			...
			self::set($env, self::structure_index, $structures);
		//	et on crée la liste de définitions des attributs
			foreach ($structures as $structure)
			{
				$attrs = $structure['attr'];
				foreach ($attrs as $attr)
				{
					self::set($env, self::attr_index, $structure['key'], $attr['key'], $attr['type']);
				}
			}
		}
	}
 ...
 */
	...
	{
	//	on charge toutes les apps
		$apps = new bunch(null, null, 'admin');
		$apps->get('app');
		$apps->set_index('key');
		self::set('admin', self::app_index, $apps);
	//	on construit la liste des classes
		$classes = array();
		foreach ($apps as $app)
		{
			$files = $app->ini('php');
		//	mise en mémoire des classes
			if (isset($files['class']))
			{
				foreach ((array) $files['class'] as $class_root)
				{
					preg_match('/([a-z0-9A-Z_-]*).php/u', $class_root, $class);
					if (isset($class[1])) $classes[$class[1]] = $app['key'];
				}
			}
		//	chargement des librairies
			if (isset($files['lib']))
			{
				foreach ((array) $files['lib'] as $lib)
				{
					require_once($app->get_root().$lib);
				}
			}
		}
