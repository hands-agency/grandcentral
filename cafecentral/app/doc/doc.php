 ...
 */
	...
	{
	//	Classes
		if ((is_object($expression) && get_class($expression)) || (is_string($expression) && class_exists($expression, false)))
		{
			$this->_prepare_class($expression);
		}
	//	Methods
		else if (mb_strpos($expression, '::'))
		{
	...
	//	Functions
		else if (is_string($expression) && function_exists($expression))
		{
		...
		//	ré-écriture des paramètres
			...
			//	récupération des informations des paramètres dans le commentaire
				if (isset($comment_params[$key]))
				{
					...
					if (isset($tmp[1])) $data['param'][$key]['type'] = $tmp[1];
					if (isset($tmp[2])) $data['param'][$key]['descr'] = $tmp[2];
				}
			}
		}
		return $data;
