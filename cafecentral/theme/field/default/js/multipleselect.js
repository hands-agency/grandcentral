...
//	Here we go!
	...
	//	The "constructor"
		...
		//	Hacky var to put up with 'out' which fires even after receive...
			var received;
			...
			$element.find('.selected ol').sortable(
			{
				...
				//	Accept otherwise
					...
					//	Add the hidden input which contains the value
						input = '<input type="hidden" name="'+plugin.settings["name"]+'[]" value="'+value+'" />';
						$(li).append(input);
						received = true;
					}
		    	}
		...
		//	Load the available choices
			$element.find('.available ul.choices').ajx(plugin.settings,
			{
			//	Callback
		...
		//	Refine on the available choices
			$element.find('.refine input[type="search"]').searchasyoutype(plugin.settings, $element.find('ul.choices'),
			{
			//	Callback
				done:function(){plugin.resort()},
			});
	...
	//	Make the available choices draggable and connected to the sortable
		plugin.resort = function()
		{
			$element.find('.available ul li').draggable(
			{
				connectToSortable:$element.find('.selected ol'),
				helper:'clone',
				revert: 'invalid',
				revertDuration: 100,
